...
def build_matrix_A(material_property_library, mesh, time):
    ...
    A[0, 0] = k[0]*Dt*4*np.pi/(rho[0]*c[0])/(DR[0]*volume)*(DR[0]/2)**2
    A[0, 1] = -A[0,0]
	

    volume = 4/3*np.pi*(mesh[0][1][1]+DR[0]/2)**3-(mesh[0][1][1]-DR[0]/2)**3
    
    A[1, 0] = -k[0]*Dt*4*np.pi/(rho[0]*c[0])/(DR[0]*volume)*(mesh[0][1][0]+DR[0]/2)**2
    A[1, 2] = -k[0]*Dt*4*np.pi/(rho[0]*c[0])/volume*(1/(1/mesh[0][1][1]-1/mesh[0][1][2]))
    A[1, 1] = -(A[1,0] +A[1,2])

	
    for i in range(2, len(mesh[0][1])-1): #Length set to fueled node length
        volume = 4/3*np.pi*((mesh[0][1][i]+DR[0]/2)**3-(mesh[0][1][i]-DR[0]/2)**3)
        A[i, i] = k[0]*Dt*4*np.pi/(rho[0]*c[0])/volume*(1/(1/mesh[0][1][i]-1/mesh[0][1][i+1]) +
                                        1/(1/mesh[0][1][i-1]-1/mesh[0][1][i]))
        A[i, i-1] = -k[0]*Dt*4*np.pi/(rho[0]*c[0])/volume*(1/(1/mesh[0][1][i-1]-1/mesh[0][1][i]))
        A[i, i+1] = -k[0]*Dt*4*np.pi/(rho[0]*c[0])/volume*(1/(1/mesh[0][1][i]-1/mesh[0][1][i+1]))
    ...
    for mat in range(1, len(mesh)):
        ...
        A[i+1,i+2] = -k[mat]*Dt*4*np.pi/(rho_c_weighted)/volume*(1/(1/mesh[mat][1][0]-1/mesh[mat][1][1]))
        A[i+1,i+1] = (k[mat-1]*Dt*4*np.pi/(rho_c_weighted)/volume*(1/(1/mesh[mat-1][1][prev_mesh_length-2]
                                                    -1/mesh[mat-1][1][prev_mesh_length-1]))+(
                                                    k[mat]*Dt*4*np.pi/(rho_c_weighted)/volume*(1/(1/mesh[mat][1][0]
                                                    -1/mesh[mat][1][1]))))
        ...
        for j in range(1, len(mesh[mat][1])-1): 
            ...
            A[j+i+1, j+i+1] = k[mat]*Dt*4*np.pi/(rho[mat]*c[mat])/volume*(1/(1/mesh[mat][1][j]-1/mesh[mat][1][j+1]) +
                                        1/(1/mesh[mat][1][j-1]-1/mesh[mat][1][j]))
            A[j+i+1, j-1+i+1] = -k[mat]*Dt*4*np.pi/(rho[mat]*c[mat])/volume*(1/(1/mesh[mat][1][j-1]-1/mesh[mat][1][j]))
            A[j+i+1, j+1+i+1] = -k[mat]*Dt*4*np.pi/(rho[mat]*c[mat])/volume*(1/(1/mesh[mat][1][j]-1/mesh[mat][1][j+1]))
        i+=len(mesh[mat][1])-1
